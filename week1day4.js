...
const partition = (arr) => {
    //set variable partition equal to partition value
    //for loop from index 0 to end of array
        //compare values to temp and move to back if less than
        //if not, 
    //
    let partitionIndex = 0;
    for(let i = 1; i < arr.length; i++){
        if(arr[i] < arr[partitionIndex]){
            for(let j = i; j > partitionIndex; j--){
                let temp = arr[j];
                arr[j] = arr[j-1];
                arr[j-1] = temp;
            }
